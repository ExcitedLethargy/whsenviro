...
//Get data
async function getFirebaseData(pDocPath = "") {
...
async function getDataFromCollection(pDocPath = "") {
    let vCollectionData = {};
    let vCollectionArray = [];
    console.log("Querying Firebase collection.");
    if (!pDocPath) {
        console.log("Error: parameter pDocPath not filled.");
        ...
        try {
            ...
            vQuerySnapshot.forEach((doc) => {
                vDocData = doc.data();
               vCollectionData[doc.id] = vDocData;
            });
...
async function writeToFirebase(pDocPath = "", pOverwriteArray = []) {
