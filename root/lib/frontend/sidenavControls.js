...
console.log("Sidenav controls active.");
...
function openSidenavFromButton(pButtonID) {
    document.getElementById("eSidenavOpener").removeEventListener('onmouseenter')
    $("#eSidenav").css('width', '20%');
    $("body").css('backgroundColor', 'rgba(0,0,0,0.4)');
    $("#eSidenav :link").css('opacity', '1');
    $("#eSidenavOpener").css({'left': '20%', 'width': '80%', 'opacity': '0'});
    $("body").css('marginLeft', '20%');
    vSidenavStatus = "open";
    $("#eSidenav").on('mouseenter', () => {
        reenableMouseDetect();
    })
}

function reenableMouseDetect() {
    $("#eSidenav").off('mouseenter');
    $("#eSidenavOpener").on('mouseenter', () => {
        toggleSidenav()
    })
}